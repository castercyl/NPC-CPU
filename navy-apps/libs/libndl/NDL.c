...
// 图像像素按行优先方式存储在`pixels`中, 每个像素用32位整数以`00RRGGBB`的方式描述颜色
  int draw_fd = _open("fb", 0, 0);
  printf("canvas_w = %d; canvas_h = %d\n", canvas_w, canvas_h);
  printf("canvas_x = %d; canvas_y = %d\n", canvas_x, canvas_y);

  for (int i = 0; i < h; i++) {
    int open_offset = _lseek(draw_fd, (((y + canvas_y + i) * screen_w) + (x + canvas_x)) * sizeof(uint32_t), SEEK_SET); //每次跨行均需重新改变读写位置
    _write(draw_fd, pixels + w * i, w * sizeof(uint32_t));
...
//(1)获取屏幕的长和宽
  ...
  int key_value;
  int proc_fd = _open("dispinfo", 0, 0);
  _read(proc_fd, proc_info, sizeof(proc_info));
  _close(proc_fd);
